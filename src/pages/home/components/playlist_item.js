...
const PlaylistItem = ({ id, title, description, playlist_thumbnail_url, duration, noVideos, category, onPlaylistClick }) => (
  <div className='o-grid__cell u-1/2@medium u-1/3@large u-1/4@extralarge u-margin-bottom-large' onClick={() => onPlaylistClick(id)}>

    <div className='c-video'>
      <div className='c-thumbnail'>
        <Link onClick={onPlaylistClick(id)} className='c-thumbnail__link' to={`/playlist/${id}`}></Link>
      ...
      <h4 className='c-video__title c-video__title--large'><Link onClick={onPlaylistClick(id)} to={`/playlist/${id}`}>{title}</Link></h4>
    </div>
  </div>
);
